...
function calculateCircleArea(radius) {
  ...
  if (typeof radius === 'number') {
    squar = Math.PI * Math.pow(radius, 2)
    return squar;
  } else {
    return null;
  }
...
function calculateVolumeCylinder(radius, height) {
  if (typeof (radius, height) === 'number') {
    ...
    let ceilrCyl = Math.ceil(cylinder);
    return ceilrCyl;
  } else {
    return null;
  }
...
function sumPositiveNumbers(numbers) {
  if (Array.isArray(numbers)) {
    ...
    for (let i = 0; i < numbers.length; i++) {
      let numCheck = Math.sign(numbers[i]);
      if (numCheck > 0) {
        posNum += numbers[i];
      }
    }
...
function getFractionalPart(num) {
  if (typeof num === 'number') {
    ...
    let otherNum = num - floorNum;
    let froundNum = Math.fround(otherNum);
    return froundNum;
  } else {
...
function compareAndRound(num1, num2) {
  if (typeof (num1, num2) === 'number') {
    ...
    let roundNum = Math.round(max);
    return roundNum;
  } else {
    return "Помилка: обидва аргументи мають бути числами.", null;
  }
...
function estimateInvestment(principal, interestRate, years) {
  if (typeof (principal, interestRate, years) === 'number') {
    let totalInvest = principal * Math.pow((1 + interestRate), years);
    let rounded = Math.round(totalInvest);
    return rounded;
  } else {
    return null;
  }
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  if (Array.isArray(products)) {
    if (typeof maxPrice === 'number') {
      let totalPrice = products.reduce((accu, currentValue) => accu + currentValue.price, 0);
      let froundTotal = Math.fround(totalPrice);
      let froundMax = Math.fround(maxPrice);
      if (froundTotal < froundMax) {
        return false;
      } else {
        return true;
      }
